...
const StockAlerts = ({ products }) => {
  ...
  useEffect(() => {
    const newAlerts = products.filter(product =>
      product.quantity <= product.minThreshold || product.quantity >= product.maxThreshold
    );
    setAlerts(newAlerts);
  }, [products]);
        ...
        <table>
          ...
          <tbody>
            {alerts.map((alert, index) => (
              <tr
                key={index}
                className={alert.quantity <= alert.minThreshold ? "critical" : "warning"}
              >
                <td>{alert.name}</td>
